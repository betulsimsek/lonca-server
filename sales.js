...
mongoose.connect(dbConnectionUrl, {
  ...
  app.get('/orders', (req, res) => {
    ...
    Order.aggregate([
      ...
      {
        $addFields: {
          year: { $year: "$payment_at" }, // Add the year field
          month: { $month: "$payment_at" } // Add the month field
        }
      },
      {
        $project: {
          ...
          year: 1, // Include the year field in the projection
          month: 1 // Include the month field in the projection
        }
      }
    ])
    .then(result => {
